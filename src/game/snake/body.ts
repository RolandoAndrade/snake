...
export class Body extends Rectangle {
    ...
    forward(positionAndVelocity: PositionAndVelocity = undefined) {
        if (!positionAndVelocity) {
            super.move(this.x + this.velocityX, this.y + this.velocityY);
        } else {
    ...
    draw(ctx: CanvasRenderingContext2D) {
        ...
        ctx.fillRect(this.x * GameConstants.SQUARE_WIDTH, this.y * GameConstants.SQUARE_WIDTH, this.width, this.height);
    }
}
